...
(function (starlingswf) {
    ...
    var Swf = (function () {
        function Swf(swfData, assetManager, fps) {
            ...
            var _this = this;
            this.createSprite = function (name, data, sprData) {
                if (data === void 0) { data = null; }
                if (sprData === void 0) { sprData = null; }
                if (sprData == null) {
                    sprData = _this._swfData[Swf.dataKey_Sprite][name];
                }
                // egret.console_log("createSprite data信息： ", sprData);
                ...
                for (var i = 0; i < length; i++) {
                    objData = sprData[i];
                    fun = _this._createDisplayFuns[objData[1]];
                    if (fun === null) {
                        continue;
                    }
                    display = fun.apply(_this, [objData[0], objData]);
                    // display = fun(this, objData[0], objData);
                    // egret.console_log("display: ", display);
                    // egret.console_log("createSprite得到图片name:", objData[0], "display: ", display);
                    display.name = objData[9];
                    display.x = objData[2];
                    display.y = objData[3];
                    // egret.console_log(`新建display111 ，x:${display.x}  y:${display.y} ,name: ${display.name}`)
                    ...
                    // egret.console_log(`新建display222 name: ${display.name} ，width:${display.width}  height:${display.height} , scaleX:${display.scaleX}  scaleY:${display.scaleY}`)
                    sprite.addChild(display);
                }
                // egret.console_log("sprite", name, "建立完成")
                return sprite;
            };
            this.createMovie = function (name, data, cls) {
                if (data === void 0) { data = null; }
                if (cls === void 0) { cls = null; }
                var movieClipData = _this._swfData[Swf.dataKey_MovieClip][name];
                ...
                for (objName in objectCountData) {
                    ...
                    for (var i = 0; i < count; i++) {
                        fun = _this._createDisplayFuns[type];
                        if (fun == null)
                            continue;
                        displayObjectArray.push(fun.apply(_this, [objName, null]));
                    }
                    displayObjects[objName] = displayObjectArray;
                }
                var mc;
                if (cls == null) {
                    mc = new starlingswf.SwfMovieClip(movieClipData["frames"], movieClipData["labels"], displayObjects, _this);
                }
                else {
                    mc = new cls(movieClipData["frames"], movieClipData["labels"], displayObjects, _this);
                }
                mc.loop = movieClipData["loop"];
                return mc;
            };
            this.createImage = function (name, data) {
                if (data === void 0) { data = null; }
                var imageData = _this._swfData[Swf.dataKey_Image][name];
                var bitmap = _this._assetManager.createBitmap(name);
                bitmap.anchorOffsetX = imageData[0];
                bitmap.anchorOffsetY = imageData[1];
                // egret.console_log('创建Image，width： ', bitmap.width, "    height: ", bitmap.height);
                return bitmap;
            };
            // public getTexture(name):egret.Texture{
            //     return this._assetManager.getTexture(name);
            // }
            this.createS9Image = function (name, data) {
                if (data === void 0) { data = null; }
                var scale9Data = _this._swfData[Swf.dataKey_Scale9][name];
                var bitmap = _this._assetManager.createBitmap(name);
            ...
            this.createShapeImage = function (name, data) {
                if (data === void 0) { data = null; }
                var bitmap = _this._assetManager.createBitmap(name);
            ...
            this.createTextField = function (name, data) {
                if (data === void 0) { data = null; }
